...
Control package for a high-voltage switching board, used for digital
microfluidics.
...
The Python package can be installed through `pip` using the following command:
    ...
    pip install wheeler.hv-switching-board
...
following command:
    ...
    python -m hv_switching_board.bin.upload <board type>
...
argument to specify the serial port to use.  For example:
    ...
    python -m hv_switching_board.bin.upload -p COM3 uno

--------------------------------------------------

## Usage ##

After uploading the firmware to the board, the `hv_switching_board.Proxy` class can be
...
### Example interactive session ###
    ...
    >>> from hv_switching_board import Proxy
...
Query descriptive properties of device.
    ...
    name                                                                  hv_switching_board
    manufacturer                                                        Wheeler Lab
    url                           http://github.com/wheeler-microfluidics/hv-switching-board
...
Below is a list of the attributes of the `hv_switching_board.Proxy` Python class.  Note
...
The Arduino firmware/sketch is located in the `hv_switching_board/Arduino/hv_switching_board`
directory.  The key functionality is defined in the `hv_switching_board::Node` class in
the file `Node.h`.
...
`hv_switching_board::Node` class in the file `Node.h`.
